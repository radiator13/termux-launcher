...
tmux set-option -g status-right "#(kew-tmux-status)#[fg=${error},bg=${surface}]#{?pane_in_mode, COPY,}#[fg=${tertiary}]#{?client_prefix, PREFIX,}#[fg=${secondary}]#{?window_zoomed_flag, ZOOM,}#[range=user|btop,fg=${primary}] #(launcher-system-monitor cpu)#[range=none] #[fg=${on_surface_variant}]│ #[range=user|btop,fg=${secondary}]#(launcher-system-monitor ram)#[range=none] #[fg=${on_surface_variant}]│ #[fg=${tertiary}]#(launcher-weather-widget) "
