...
tmux set-option -g status-right "#[fg=${error},bg=${surface}]#{?pane_in_mode, COPY,}#[fg=${tertiary}]#{?client_prefix, PREFIX,}#[fg=${secondary}]#{?window_zoomed_flag, ZOOM,}#[range=user|btop,fg=${primary}] #(launcher-system-monitor cpu)#[range=none] #[fg=${outline_variant}]│ #[range=user|btop,fg=${secondary}]#(launcher-system-monitor ram)#[range=none] #[fg=${outline_variant}]│ #[fg=${tertiary}]#(launcher-weather-widget) "
